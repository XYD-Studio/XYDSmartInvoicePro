...
const keys = ref({
  ...
  custom: store.config.keys?.custom || '' // 自定义模型的 Key
})
  ...
  try {
    if (provider.value === 'aliyun-ocr') {
      ...
      if (provider.value === 'custom') {
        if (!customBaseUrl.value.trim() || !customModel.value.trim()) return alert('请填写完整的自定义 BaseURL 和 Model')
    ...
    const res = await axios.post('http://127.0.0.1:8000/api/v1/test-api', formData)
...
const saveConfig = () => {
  if (provider.value === 'aliyun-ocr') {
    if (!aliyunAk.value.trim() || !aliyunSk.value.trim()) return alert('⚠️ 选用 OCR 引擎必须填写完整的 AccessKey ID 和 Secret！')
  } else if (provider.value === 'custom') {
    if (!currentKey.value.trim() || !customBaseUrl.value.trim() || !customModel.value.trim()) {
      return alert('⚠️ 自定义模式必须填写完整的 BaseURL、模型名和 Key！')
    }
  } else {
      ...
      <div class="p-6 overflow-y-auto space-y-6 flex-1">
        ...
        <div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 mb-3">
            ...
            <label class="border rounded-xl p-3 cursor-pointer transition-colors" :class="provider === 'zhipu' ? 'border-indigo-500 bg-indigo-50 ring-1 ring-indigo-500' : 'border-gray-200 hover:border-indigo-300'">
              ...
              <div class="font-bold text-sm text-indigo-800">🤖 智谱 (GLM-4V)</div>
        ...
        <div class="space-y-4 border-t pt-4">
           ...
           <div class="bg-gray-50 p-3 rounded-lg text-sm text-gray-700 border border-gray-200 flex justify-between items-center">
             <span v-if="provider === 'aliyun-ocr'">去 <a href="https://ram.console.aliyun.com/manage/ak" target="_blank" class="font-bold text-purple-600 hover:underline">阿里云 RAM 控制台</a> 生成 AK/SK，需开通【增值税发票识别】权限。</span>
             <span v-if="provider === 'aliyun'">去 <a href="https://dashscope.console.aliyun.com/api-key-management" target="_blank" class="font-bold text-blue-600 hover:underline">百炼控制台</a> 获取通义千问 API Key。</span>
             <span v-if="provider === 'zhipu'">去 <a href="https://open.bigmodel.cn/usercenter/apikeys" target="_blank" class="font-bold text-indigo-600 hover:underline">智谱 AI 开放平台</a> 注册并获取 API Key。</span>
             <span v-if="provider === 'openai'">去 <a href="https://platform.openai.com/api-keys" target="_blank" class="font-bold text-emerald-600 hover:underline">OpenAI Platform</a> 生成 API Key。</span>
             <span v-if="provider === 'custom'" class="text-orange-700 font-medium">⚠️ 警告：您配置的第三方模型必须具备<b>图像视觉(Vision)能力</b>，否则无法提取发票信息！</span>
           ...
           <div v-if="provider === 'aliyun-ocr'">
              ...
              <input type="password" v-model="aliyunAk" class="w-full border-gray-300 rounded-lg focus:ring-purple-500 py-2 mb-3" />
              
              <label class="block text-xs font-semibold text-gray-700 mb-1">AccessKey Secret</label>
              <input type="password" v-model="aliyunSk" class="w-full border-gray-300 rounded-lg focus:ring-purple-500 py-2" />
           </div>
           ...
           <div v-else-if="provider === 'custom'" class="space-y-3 bg-orange-50/30 p-4 rounded-lg border border-orange-100">
              <div>
                ...
                <input type="text" v-model="customBaseUrl" placeholder="如: https://api.moonshot.cn/v1" class="w-full border-gray-300 rounded-lg focus:ring-orange-500 py-2 text-sm" />
              </div>
              <div>
                <label class="block text-xs font-semibold text-gray-700 mb-1">模型名称 (Model Name)</label>
                <input type="text" v-model="customModel" placeholder="如: moonshot-v1-32k-vision" class="w-full border-gray-300 rounded-lg focus:ring-orange-500 py-2 text-sm" />
              </div>
              <div>
                ...
                <div class="relative">
                  <input :type="showKey ? 'text' : 'password'" v-model="currentKey" class="w-full border-gray-300 rounded-lg focus:ring-orange-500 py-2 pr-10 text-sm" />
           ...
           <div v-else>
              ...
              <div class="flex space-x-2">
                <div class="relative flex-1">
                  <input :type="showKey ? 'text' : 'password'" v-model="currentKey" class="w-full border-gray-300 rounded-lg focus:ring-blue-500 py-2 pr-10 text-sm" @keyup.enter="saveConfig"/>
                  <button @click="showKey = !showKey" class="absolute right-3 top-2 text-gray-400">{{ showKey ? '👁️' : '🙈' }}</button>
                </div>
